...
class Secret_key(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.rules = None
        self.entropie = None
        #self.ui.radio_random_rules.setChecked(True)
        #self.ui.radio_specified_rules.setChecked(True)
    ...
    def encrypt_clicked(self):  # zmienai na postac binarna jesli klucz nie jest pusty i robi XOR
        ...
        # TWOJ KOD SIE ZACZYNA STAD, U GORY MASZ MOJEGO IFA JAK TO ROZLOZYC NA DWA PRZYPADKI PRZY
        entropia = 0
    ...
    #tego juz nie wykorzystujemy
    '''
    def nielosowe_zera_i_jedynki(self):
        lista = []
        tmp = self.ui.initial_state.text()
        for licznik in range(
                len(self.ui.initial_state.text())):  # to zwraca losowy stan poczatkowy jezeli user nie wpisal
    ...
    def liczymy_ph(self, ministring, maxistring):  ### 1101 101010100101011101010010110
        ...
        licznik_czesci = len(maxistring) - (
                    len(ministring) - 1)  # to zamiast petli bo kazda liczba oprocz trzech ostatnich ma swoja czworke
        # to wyzej moze przerobic na: int"(self.ui.h_box.currentText())-1"
        # zamiast -3 na sztywno, bo z 2 sie wykrzaczy
        while i < licznik_czesci:
            if ministring == maxistring[i:i + len(
                    ministring)]:  # ministring bez nawiasow bo zawsze ma 4, TO CHYBA TEZ TRZEBA ZMIENIC
                powtorzona_czesc += 1
            i += 1
